...
{
    /// <summary>
    /// World-upright plate under a drone mesh: thin health bar + owner badge,
    /// same chrome as <see cref="PeopleTransportNameplate"/>. Unparented so hull
    /// yaw / buzz facing does not spin the bar. Screen-below (world −Z).
    /// HP is ghosted <c>EquippedEquipmentElement.RemainingCharges</c>.
    /// </summary>
    [DefaultExecutionOrder(67030)]
    public sealed class DroneSwarmNameplate : MonoBehaviour
    {
        const int BarSortingOrder = 5000;
        const int BadgeSortingOrder = 5004;
        /// <summary>Local bar width; LateUpdate scales the root so this equals ~2× drone width.</summary>
        const float BarWidth = 10f;
        const float BarHeight = 1.7f;
        const float BadgeSize = 4f;
        const float PadPastHull = 0.10f;
        const float MinDroneWidth = 0.22f;
        ...
        public static DroneSwarmNameplate Ensure(GameObject host)
        {
            if (host == null)
                return null;
            var plate = host.GetComponent<DroneSwarmNameplate>();
            if (plate == null)
                plate = host.AddComponent<DroneSwarmNameplate>();
            plate.EnsureHierarchy();
            return plate;
        }

        /// <summary>
        /// Owner badge + live HP. Max is <see cref="StoreItemData.GetDroneMaxHp"/>.
        /// </summary>
        public static void Sync(GameObject host, int networkId, int remainingCharges, int maxHp)
        {
            if (host == null)
                return;
            var plate = host.GetComponent<DroneSwarmNameplate>();
            ...
            float hp = Mathf.Clamp(remainingCharges, 0f, max);
        ...
        /// <summary>Snap the unparented plate after the drone mesh pose this frame.</summary>
        ...
        public void FollowHostNow()
        {
            ...
            float clearance = diameter * 0.5f + PadPastHull
                + DroneSwarmPositioning.DroneHitSphereRadius * 0.35f;
        ...
        void EnsureHierarchy()
        {
            ...
            var rootGo = new GameObject("DroneSwarmNameplate");
        ...
        static Sprite GetBarSprite()
        {
            ...
            tex.name = "DroneSwarmNameplateBarTex";
            ...
            s_BarSprite.name = "DroneSwarmNameplateBarSprite";
            return s_BarSprite;
        }
        ...
        static Material GetPlayerBadgeMaterial()
        {
            ...
            s_PlayerBadgeMaterial.name = "DroneSwarmNameplateBadge";
