...
namespace TitanOrbit.Simulation
{
    ...
    public static class GemTractorBeamAssignment
    {
            ...
            int maxCooperatingBeams)
        {
            ...
            if (wingToGemLocks.Count > 0)
            {
                ...
                for (int i = 0; i < stickyWings.Count; i++)
                {
                    ...
                    if (gemBeamCountScratch.TryGetValue(gemId, out int existing) && existing >= maxBeams)
                        continue;
            ...
            if (maxBeams > 1)
            {
                ...
                for (int i = 0; i < filteredScratch.Count; i++)
                {
                    ...
                    if (wingClaimed[c.WingIndex])
                        continue;
                    if (gemBeamCountScratch.TryGetValue(c.GemId, out int count) && count >= maxBeams)
                        continue;
        ...
        /// <summary>Increments how many beams already claim <paramref name="gemId"/>.</summary>
        ...
        /// <summary>Packs wing + gem into one HashSet key (no nested dictionary alloc).</summary>
        static long PackWingGem(int wingIndex, int gemId) => ((long)wingIndex << 32) | (uint)gemId;

        /// <summary>
        /// Shortest distance first, then stable wing/gem ids for deterministic ties
