...
namespace TitanOrbit.Systems
{
    ...
    public class CardShopSystem : NetworkBehaviour
    {
        ...
        /// <summary>True when an AI ship has enough contributed gems at home to buy the next hull tier.</summary>
        public bool CanAiAffordShipLevelUpgrade(Starship ship, Planet storePlanet, out float cost)
        {
            cost = 0f;
            if (ship == null || storePlanet == null) return false;
            if (!CanPurchaseShipLevelUpgrade(ship, storePlanet, out int nextLevel, out cost, out _))
                return false;
