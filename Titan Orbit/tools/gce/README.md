...
### Atomic install (deploys can't break the running server)

Both deploy paths now stage the new build into a sibling `…/TitanOrbitLinux1.new` directory, verify the IL2CPP runtime sizes there, and only then atomically rename it over the live install. The relevant logic is in [`install_linux_build_from_gcs_on_vm.sh`](install_linux_build_from_gcs_on_vm.sh) and the inline install script inside [`upload_linux_build_to_gce_openssh.ps1`](upload_linux_build_to_gce_openssh.ps1). The service is stopped before the swap so `Restart=on-failure` cannot bind onto a half-extracted tree mid-deploy. If a verify check fails (typical cause: Unity left `global-metadata.dat` as a 0-byte stub during `tar`), the staging tree is deleted and the previously installed build keeps running.

### One-command diagnose from Windows

Run [`diagnose_titanorbit_server_on_gce.bat`](diagnose_titanorbit_server_on_gce.bat) (or `diagnose_titanorbit_server_on_gce.bat useIap`) from `tools\gce` to execute [`diagnose_titanorbit_server_vm.sh`](diagnose_titanorbit_server_vm.sh) on the VM via `gcloud compute ssh` and stream its output back to your console. The script reports unit status, IL2CPP file sizes, `Player.log` tail, `journalctl` tail, and `ldd GameAssembly.so` missing libraries — the same evidence the troubleshooting sections below ask for, gathered in one round-trip.
...
| `diagnose_titanorbit_server_on_gce.bat` / `.ps1` | Windows launcher that runs **`diagnose_titanorbit_server_vm.sh`** on the VM via **`gcloud compute ssh`** (optional **`useIap`**). Prints unit status, **IL2CPP** file sizes, **`Player.log`** tail, **`journalctl`** tail, and **`ldd`** missing libraries — one round trip for the evidence the troubleshooting sections ask for. |
