    ...
    public partial struct GhostSpawnSystem : ISystem
    {
        ...
        public unsafe void OnUpdate(ref SystemState state)
        {
            ...
            for (int i = 0; i < ghostSpawnBuffer.Length; ++i)
            {
                ...
                // TITAN-ORBIT: guard invalid GhostType (hash mismatch / stale collection) — avoids Burst OOB hard-crash.
            ...
            spawnedGhosts.Clear();
            // TITAN-ORBIT: Cap Instantiates per frame.
            // Late-join map ghosts share past ClientSpawnTick values, so the stock while-loops would
            // Instantiate an entire asteroid/planet backlog in one Burst update (Windows hard-crash).
            // Leave the rest in the delayed queues for the next frames — placeholders stay mapped.
            const int k_MaxDelayedInstantiatesPerFrame = 12;
            int delayedInstantiates = 0;
            while (delayedInstantiates < k_MaxDelayedInstantiatesPerFrame &&
        ...
        unsafe bool TrySpawnFromDelayedQueue(ref SystemState state, in DelayedSpawnGhost ghost, GhostSpawnBuffer.Type spawnType, in NativeArray<GhostCollectionPrefab> prefabs, Entity ghostCollectionSingleton, out Entity entity)
        {
            ...
            // TITAN-ORBIT: bounds-check before Instantiates (Burst OOB → native Crash!!! on Windows).
