...
## Current patch (`TO_GhostSpawn_v16_requeueFailedInstantiate`)
...
3. **16 Instantiates/frame** from delayed queues (was 1; asteroids are seed-hydrated now, remaining ghosts are ships/planets/gems).
...
5. **Predicted join path** — delayed Instantiates via placeholders; predicted ships Instantiates before interpolated gems/planets.
6. **`TitanOrbitJoinLoadCounters`** — Instantiates/placeholder totals for loading UI without asteroid gathers.
7. **Server companion:** distance `TileSize` ≪ map size (~48). Transform stays **ON** (seed-hydrate). Do not drop `MaxSendChunks` to 1 — that starved gem snapshots.

## Companion project systems (not in this file)

- Server: `TitanOrbitGhostSendTuneSystem` + `TitanOrbitGhostDistanceImportanceSystem` + gem spatial relevancy / tractor pin.
- Client: `TitanOrbitClientJoinTransformGateSystem` + `ClientJoinSettleCache` (Settling; Transform stays on).
- Hybrid: `EcsWorldVisualizer` Pending drain.
...
`TO_GhostSpawn_v16_requeueFailedInstantiate`
